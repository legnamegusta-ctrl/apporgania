...
export function initAdminDashboard(userId, userRole) {
  ...
  const switchTab = (targetId) => {
    ...
    switch (targetId) {
      case "dashboard":
        if (!isDashboardTabInitialized) initDashboardTab()
        break
      case "clientes":
        if (!isClientesTabInitialized) initClientesTab()
        break
      case "producao":
        if (!isProducaoTabInitialized) initProducaoTab(userId, userRole)
        break
      case "ferramentas":
        if (!isFerramentasTabInitialized) initFerramentasTab(userId, userRole)
        break
    }
  }
  ...
  async function initDashboardTab() {
    ...
    try {
      ...
      const agronomos = usersSnapshot.docs
        .map((doc) => ({ id: doc.id, ...doc.data() }))
        .filter((u) => u.role === "agronomo")
      if (totalAgronomosStat) totalAgronomosStat.textContent = agronomos.length

      const totalTonnage = salesSnapshot.docs.reduce(
        (sum, doc) => sum + (doc.data().items || []).reduce((itemSum, item) => itemSum + (item.tonnage || 0), 0),
        0,
      )
    ...
    function renderClientsByAgronomoChart(clients, agronomos) {
      ...
      const data = {
        labels: [],
        datasets: [{ label: "Clientes", data: [], backgroundColor: brandColors, hoverOffset: 4 }],
      }
      const clientsByAgronomo = {}
      agronomos.forEach((agronomo) => {
        clientsByAgronomo[agronomo.name || `Agrônomo ${agronomo.id.substring(0, 5)}`] = 0
      })
      clientsByAgronomo["Sem Agrônomo"] = 0
      clients.forEach((c) => {
        const agronomoDoc = agronomos.find((u) => u.id === c.agronomistId)
        const agronomoName = agronomoDoc
          ? agronomoDoc.name || `Agrônomo ${agronomoDoc.id.substring(0, 5)}`
          : "Sem Agrônomo"
        if (clientsByAgronomo[agronomoName] !== undefined) {
          clientsByAgronomo[agronomoName]++
        }
      })
      for (const [agronomoName, count] of Object.entries(clientsByAgronomo)) {
        if (count > 0) {
          data.labels.push(agronomoName)
  ...
  async function initClientesTab() {
    ...
    const availableModules = [
      { id: "reports", name: "Relatório de Evolução" },
      { id: "photos", name: "Galeria de Fotos do Talhão" },
    ]
    ...
    async function loadAllClientsForManagement() {
      if (adminClientsList) showSpinner(adminClientsList)
      try {
        ...
        if (adminClientsList)
          adminClientsList.innerHTML =
            '<tr><td colspan="4" class="text-center py-4 text-red-500">Ocorreu um erro ao buscar os clientes.</td></tr>'
      } finally {
        if (adminClientsList) hideSpinner(adminClientsList)
      }
    }
    ...
    function renderAdminClientList(clientsToRender) {
      ...
      if (clientsToRender.length === 0) {
        adminClientsList.innerHTML =
          '<tr><td colspan="4" class="text-center py-4 text-gray-500">Nenhum cliente encontrado.</td></tr>'
        return
      }
      clientsToRender.forEach((client) => {
        const assignedAgronomist = allAgronomists.find((a) => a.id === client.agronomistId)
        const statusBadge = client.clientAuthUid
        ...
        const linkButton = !client.clientAuthUid
          ? `<button data-action="setup-access" data-client-id="${client.id}" data-client-name="${client.name}" class="text-purple-600 hover:text-purple-900" title="Vincular Usuário"><i class="fas fa-link"></i></button>`
          : ""
    ...
    async function openClientModal(clientId = null) {
      ...
      const titleEl = clientModal ? clientModal.querySelector("#clientModalTitle") : null
      const nameEl = clientModal ? clientModal.querySelector("#clientModalName") : null
      const agroEl = clientModal ? clientModal.querySelector("#clientModalAgronomist") : null
      ...
      if (clientId) {
        ...
        if (titleEl) titleEl.textContent = "Adicionar Novo Cliente"
    ...
    async function handleSaveClient() {
      ...
      const agronomistId = agroEl ? agroEl.value : ""
      if (!name || !agronomistId) {
        showToast("Nome do cliente e agrônomo são obrigatórios.", "error")
        return
      }

      const saveBtn = clientModal ? clientModal.querySelector('button[data-action="save"]') : null
      if (saveBtn) {
    ...
    async function openModulesModal(buttonElement) {
      ...
      const clientNameEl = modulesModal ? modulesModal.querySelector("#modulesModalClientName") : null
      if (clientNameEl) clientNameEl.textContent = `Cliente: ${buttonElement.dataset.clientName}`
      const listEl = modulesModal ? modulesModal.querySelector("#modulesList") : null
      if (listEl) showSpinner(listEl)
    ...
    async function handleSaveModules() {
      ...
      const saveBtn = modulesModal ? modulesModal.querySelector('button[data-action="save"]') : null
      if (saveBtn) saveBtn.disabled = true
      const enabledModules = {}
      const checkboxes = modulesModal ? modulesModal.querySelectorAll('input[type="checkbox"]') : []
    ...
    function openAccessModal(buttonElement) {
      currentEditingClientId = buttonElement.dataset.clientId
      const clientNameEl = accessModal ? accessModal.querySelector("#accessModalClientName") : null
      const uidInput = accessModal ? accessModal.querySelector("#clientUidInput") : null
      if (clientNameEl) clientNameEl.textContent = `Cliente: ${buttonElement.dataset.clientName}`
    ...
    async function handleLinkClientUid() {
      const uidInput = accessModal ? accessModal.querySelector("#clientUidInput") : null
      const saveBtn = accessModal ? accessModal.querySelector('button[data-action="save"]') : null
      const uid = uidInput ? uidInput.value.trim() : ""
      if (!uid || uid.length < 28) {
        showToast("Por favor, insira um UID válido do Firebase.", "error")
        return
      }
    ...
    if (adminClientSearch)
      adminClientSearch.addEventListener("input", () =>
        renderAdminClientList(
          allClientsForAdmin.filter((c) => c.name.toLowerCase().includes(adminClientSearch.value.toLowerCase())),
        ),
      )
    if (adminAddClientBtn) adminAddClientBtn.addEventListener("click", () => openClientModal())
  ...
  async function loadAllAgronomists() {
    try {
      ...
      const usersSnapshot = await getDocs(
        query(collection(db, "users"), where("role", "==", "agronomo"), orderBy("name")),
      )
      allAgronomists = usersSnapshot.docs.map((doc) => ({
        id: doc.id,
        name: doc.data().name || `Agrônomo ${doc.id.substring(0, 5)}`,
      }))
    } catch (error) {
