...
import { collection, query, where, orderBy, getDocs, doc, updateDoc, addDoc, serverTimestamp, onSnapshot, increment, getDoc } from 'https://www.gstatic.com/firebasejs/9.6.0/firebase-firestore.js';
...
export function initPlotDetails(userId, userRole) {
    ...
    async function buildTimeline() {
        ...
        try {
            ...
            allTimelineItems.forEach(item => { 
                ...
                // Tenta atribuir um nome de responsável, priorizando registeredByName, depois registeredById, senão Agrônomo/Operador Desconhecido
                ...
                } else if (item.responsibleAgronomistId && allUsersCache[item.responsibleAgronomistId]) { // Para tarefas
    ...
    function renderTimelineItems() {
        ...
        itemsToRender.forEach(item => {
            ...
            // Adiciona data-task-id se existir
            ...
            // Determina a classe para o cartão e o tipo de evento para o handler
            const cardClass = isAnalysis ? 'analysis-card' : 'management-card';
            const dataEventType = item.eventGroup; // 'analysis' ou 'management'
            const dataEventId = item.id; 

            // Determina a cor do status da atividade
            let statusBadgeHtml = '';
            if (item.status) {
            ...
            // Fotos (apenas para manejos, por enquanto)
            ...
            if (isAnalysis) {
                ...
                // Adiciona a imagem no corpo da descrição apenas se for foto_talhao E houver a primeira imagem
            ...
            // Adiciona todas as fotos de manejos para a galeria (se o módulo estiver habilitado)
    ...
    function openViewAnalysisModal(analysisId) {
        ...
        // Oculta/mostra o botão de edição baseado no userRole
    ...
    async function handleAddManagement() {
        ...
        const managementData = {
            ...
            imageUrls: [] // Assume array de URLs
        };
        ...
        try {
            ...
            closeModal(managementModal); // LÓGICA DE MODAL ATUALIZADA
            buildTimeline();
    ...
    function addEventListeners() {
        // Botões de Adicionar/Editar (visibilidade já controlada no loadHeaderData)
        ...
        if (timelineContainer) timelineContainer.addEventListener('click', (e) => {
            ...
            // Para análises, o clique pode ser no card inteiro ou no botão
            const analysisCard = e.target.closest('.analysis-card'); 

            if (photoTarget) {
                openLightbox(photoTarget.dataset.src);
            } else if (detailsBtn) { // Prioriza o clique no botão "Ver Detalhes"
                const itemContainer = detailsBtn.closest('[data-event-type]'); // Encontra o container do evento
                const eventType = itemContainer.dataset.eventType;
                const eventId = itemContainer.dataset.eventId;
                const taskId = itemContainer.dataset.taskId; // Pega o taskId, se existir

                if (taskId) {
                    window.location.href = `task-viewer.html?taskId=${taskId}&clientId=${clientId}&propertyId=${propertyId}&plotId=${plotId}&view=details`; // Inclui propertyId e plotId
                } else if (eventType === 'analysis') {
                    openViewAnalysisModal(eventId);
                } else if (eventType === 'management') { // Manejos que não são foto_talhao e não tem taskId
                ...
                // Fallback para clique no card de análise se não clicou no botão e não é cliente/operador
    ...
    function setupBottomNavbarListeners() {
        ...
        if (navHomeBtn) {
            navHomeBtn.addEventListener('click', (e) => {
                e.preventDefault();
                // Home vai para o dashboard apropriado
