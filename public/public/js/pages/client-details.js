...
export function initClientDetails(userId, userRole) {
  ...
  const closeAddPropertyModalBtn = addPropertyModal ? addPropertyModal.querySelector("#closeAddPropertyModalBtn") : null
  const savePropertyBtn = addPropertyModal ? addPropertyModal.querySelector("#addPropertyBtn") : null
  const newPropertyNameInput = addPropertyModal ? addPropertyModal.querySelector("#newPropertyName") : null

  const addTaskModal = document.getElementById("addTaskModal")
  const closeAddTaskModalBtn = addTaskModal ? addTaskModal.querySelector("#closeAddTaskModalBtn") : null
  const addClientTaskBtn = addTaskModal ? addTaskModal.querySelector("#addClientTaskBtn") : null
  const modalTaskTitle = addTaskModal ? addTaskModal.querySelector("#modalTaskTitle") : null
  const modalTaskDescription = addTaskModal ? addTaskModal.querySelector("#modalTaskDescription") : null
  const modalTaskDate = addTaskModal ? addTaskModal.querySelector("#modalTaskDate") : null
  const modalTaskProperty = addTaskModal ? addTaskModal.querySelector("#modalTaskProperty") : null
  const modalTaskPlot = document.getElementById("modalTaskPlot") // Obtido diretamente do DOM

  const saleModal = document.getElementById("saleModal")
  const closeSaleModalBtn = saleModal ? saleModal.querySelector("#closeSaleModalBtn") : null
  const addSaleItemBtn = saleModal ? saleModal.querySelector("#addSaleItemBtn") : null
  const saveSaleBtn = saleModal ? saleModal.querySelector("#saveSaleBtn") : null
  const saleItemsContainer = saleModal ? saleModal.querySelector("#saleItemsContainer") : null
  ...
  async function initializePage() {
    ...
    if (typeof L === "undefined") {
      console.error(
        "Leaflet (L) não está definido ao tentar inicializar o mapa. Certifique-se de que o script Leaflet foi carregado.",
      )
  ...
  async function loadLoggedInUserData() {
    try {
      ...
      if (userDoc.exists()) {
        ...
        throw new Error("Documento do usuário logado não encontrado.")
      }
    } catch (error) {
  ...
  function createSaleItemElement() {
    ...
    if (fixedFormulas.length > 0) {
      optionsHtml += `<optgroup label="Fórmulas Fixas">${fixedFormulas.map((f) => `<option value="${f.name}">${f.name}</option>`).join("")}</optgroup>`
    }
    if (customFormulas.length > 0) {
      optionsHtml += `<optgroup label="Fórmulas Personalizadas">${customFormulas.map((f) => `<option value="${f.name}">${f.name}</option>`).join("")}</optgroup>`
    }
  ...
  function saveSale() {
    ...
    if (saleItems.length === 0) {
      showToast("Adicione pelo menos uma formulação à venda.", "error")
      return
    }
    ...
    getDocs(adminQuery)
      .then((adminSnapshot) => {
        ...
        saleItems.forEach((item) => {
          ...
          if (formulaName === "outra") {
            formulaName = item.querySelector(".custom-formula-input").value.trim()
          }
          const tonnage = Number.parseFloat(tonnageInput.value)
          if (!formulaName || isNaN(tonnage) || tonnage <= 0) {
            isValid = false
          }
          saleData.items.push({ formulaName, tonnage })
        })
        if (!isValid) {
          throw new Error("Verifique os itens da venda.")
        }
  ...
  async function openPropertyModal(propertyId = null) {
    ...
    if (newPropertyNameInput) newPropertyNameInput.value = ""
    ...
    if (propertyId) {
      ...
      if (propertyDoc.exists()) {
        ...
        if (newPropertyNameInput) newPropertyNameInput.value = propertyData.name
  ...
  async function handleSaveProperty() {
    if (!newPropertyNameInput) return
    const name = newPropertyNameInput.value.trim()
    if (!name) {
      showToast("O nome da propriedade é obrigatório.", "error")
      return
    }
  ...
  function handleRemoveLocation() {
    ...
    tempCoordinates = null
    if (coordinatesDisplay) coordinatesDisplay.textContent = "Localização removida."
    if (marker) {
      marker.remove()
      marker = null
    }
  ...
  function handleAddTask() {
    if (!modalTaskTitle || !modalTaskDate) return
    ...
    const taskData = {
      title: title,
      description: modalTaskDescription ? modalTaskDescription.value.trim() || null : null,
      dueDate: dueDate,
      propertyId: modalTaskProperty ? modalTaskProperty.value || null : null,
      plotId: modalTaskPlot ? modalTaskPlot.value || null : null,
      isCompleted: false,
      createdAt: serverTimestamp(), // Uso corrigido de serverTimestamp
    }
  ...
  function useCurrentLocation() {
    ...
    if (getUserLocationBtn) getUserLocationBtn.disabled = true
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (getUserLocationBtn) getUserLocationBtn.disabled = false
      },
      () => {
  ...
  function addEventListeners() {
    if (showSaleModalBtn)
      showSaleModalBtn.addEventListener("click", () => {
    ...
    if (saleItemsContainer) {
      saleItemsContainer.addEventListener("change", (e) => {
        if (e.target.classList.contains("formula-select")) {
          const customContainer = e.target.closest(".sale-item").querySelector(".custom-formula-container")
          customContainer.classList.toggle("hidden", e.target.value !== "outra")
        }
      })
    ...
    if (showAddPropertyModalBtn)
      showAddPropertyModalBtn.addEventListener("click", () => {
      ...
      openMapModalBtn.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          if (tempCoordinates) {
            ...
            if (!marker) {
              // Adiciona marcador se não houver um
    ...
    if (confirmMapSelectionBtn)
      confirmMapSelectionBtn.addEventListener("click", () => {
        if (tempCoordinates && coordinatesDisplay) {
          coordinatesDisplay.textContent = `Lat: ${tempCoordinates.lat.toFixed(5)}, Lng: ${tempCoordinates.lng.toFixed(5)}`
        }
        closeModal(mapModal)
      })
