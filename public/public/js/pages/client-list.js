...
export function initClientList(userId, userRole) {
    ...
    clientsToRender.forEach((client) => {
      const checkboxHtml =
        client.status !== "inativo"
          ? `<input type="checkbox" class="client-sync-checkbox h-5 w-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" data-client-id="${client.id}">`
          : ""
      let actionButtons
      if (client.status === "inativo") {
        ...
        const isFavoriteIcon = client.isFavorite
  ...
  const handleFilter = () => {
    ...
    if (currentFilter === "archived") {
      ...
      const activeClients = allMyClients.filter((c) => c.status !== "inativo")
  ...
  function updateSyncSelection() {
    ...
    if (count > 0) {
      ...
      if (syncSelectionCount) syncSelectionCount.textContent = `${count} cliente(s) selecionado(s).`
      if (syncOfflineDataBtn) syncOfflineDataBtn.disabled = false
    } else {
      if (syncSection) syncSection.classList.add("hidden")
      if (syncOfflineDataBtn) syncOfflineDataBtn.disabled = true
    }
  }
  ...
  async function syncSelectedClients() {
    ...
    if (syncStatus) syncStatus.textContent = "Iniciando sincronização..."
    ...
    try {
      ...
      if (syncStatus) syncStatus.textContent = "Erro na sincronização."
    } finally {
      if (syncOfflineDataBtn) {
  ...
  function setupEventListeners() {
    ...
    if (agronomistClientsList) {
      agronomistClientsList.addEventListener("click", (e) => {
        ...
        if (archiveButton) {
          archiveClient(archiveButton.dataset.clientId)
          return
        }
        const unarchiveButton = e.target.closest(".unarchive-btn")
        if (unarchiveButton) {
          unarchiveClient(unarchiveButton.dataset.clientId)
          return
        }
