...
import * as L from "https://unpkg.com/leaflet@1.7.1/dist/leaflet.js" // Import Leaflet
...
export function initMapaAgronomo(userId, userRole) {
  ...
  if (!mapContainer) {
    ...
    if (mainElement) {
      mainElement.innerHTML =
        '<div class="text-center p-8 bg-red-100 text-red-800 rounded-lg shadow-md"><p class="font-bold">Erro!</p><p>Não foi possível encontrar o local para o mapa. Por favor, recarregue a página.</p></div>'
    }
    return
  }
  ...
  async function initializeMapaAgronomoPage() {
    ...
    const allElementsPresent =
      ...
      observationText &&
      observationImage &&
      closeObservationModalBtn &&
      saveObservationBtn &&
      currentVisitClientName &&
      currentVisitPropertyName &&
      closeVisitPanelBtn &&
      addObservationBtn &&
      endVisitBtn
  ...
  async function loadAgronomistClientsProperties() {
    try {
      const clientsSnapshot = await getDocs(query(collection(db, "clients"), where("agronomistId", "==", userId)))
      ...
      mapContainer.innerHTML =
        '<p class="text-center text-red-500 p-8">Ocorreu um erro ao carregar os dados do mapa dos seus clientes.</p>'
      if (agronomistClientsLeafletMap) {
        agronomistClientsLeafletMap.remove()
        agronomistClientsLeafletMap = null
      }
      if (error.code === "failed-precondition") {
        showToast(
          "Um índice do Firestore é necessário para esta consulta de mapa. Verifique o console para o link de criação.",
          "error",
          10000,
        )
      } else if (error.code === "permission-denied") {
        showToast(
          "Erro de permissão ao carregar dados dos clientes para o mapa. Verifique as regras de segurança.",
          "error",
          10000,
        )
      }
  ...
  function filterAndRenderProperties() {
    const selectedClientId = filterClientMap ? filterClientMap.value : ""
    const selectedStatus = filterStatusMap ? filterStatusMap.value : "all" // 'all' ou 'active'
    ...
    if (filteredProperties.length === 0) {
      mapContainer.innerHTML =
        '<p class="text-center text-gray-500 p-8">Nenhuma propriedade encontrada com os filtros selecionados.</p>'
      if (agronomistClientsLeafletMap) {
  ...
  function initializeAgronomistClientsMap() {
    ...
    if (
      agronomistClientsLeafletMap instanceof L.Map &&
      agronomistClientsLeafletMap._container &&
      agronomistClientsLeafletMap._container.id === "agronomistClientsMap"
    ) {
      return
    }
  ...
  function locateUser() {
    ...
    if (locateMeBtn) locateMeBtn.disabled = true
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (locateMeBtn) locateMeBtn.disabled = false
      },
      (error) => {
  ...
  async function startVisit(clientId, propertyId, propertyName, clientName, lat, lng) {
    ...
    try {
      ...
      if (error.code === "permission-denied") {
        showToast(
          "Erro de permissão ao iniciar visita. Verifique as regras de segurança para a coleção 'visits'.",
          "error",
          10000,
        )
      }
  ...
  async function endVisit() {
    ...
    if (endVisitBtn) endVisitBtn.disabled = true
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (endVisitBtn) endVisitBtn.disabled = false
        }
      },
      (error) => {
  ...
  async function addObservation() {
    ...
    const text = observationText ? observationText.value.trim() : ""
    const imageUrl = observationImage ? observationImage.value.trim() : ""
    ...
    if (saveObservationBtn) saveObservationBtn.disabled = true
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (observationText) observationText.value = ""
          if (observationImage) observationImage.value = ""
          closeModal(addObservationModal)
        } catch (error) {
          ...
          if (saveObservationBtn) saveObservationBtn.disabled = false
        }
      },
      (error) => {
  ...
  function updateVisitPanelInfo() {
    if (currentVisitInProgress) {
      if (currentVisitClientName) currentVisitClientName.textContent = currentVisitInProgress.clientName
      if (currentVisitPropertyName) currentVisitPropertyName.textContent = currentVisitInProgress.propertyName
    }
  }
  ...
  function setupGeneralEventListeners() {
    ...
    if (closeObservationModalBtn)
      closeObservationModalBtn.addEventListener("click", () => closeModal(addObservationModal))
  ...
  function setupMapSpecificEventListeners() {
    ...
    if (agronomistClientsLeafletMap) {
      agronomistClientsLeafletMap.on("popupopen", (e) => {
        const popup = e.popup
        ...
        if (popup && popup._contentNode) {
          ...
          if (startVisitButton) {
            startVisitButton.onclick = () => {
              ...
              startVisit(clientId, propertyId, propertyName, clientName, Number.parseFloat(lat), Number.parseFloat(lng))
            }
          }
        }
      })
