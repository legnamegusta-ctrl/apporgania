...
export function initAgronomoDashboard(userId, userRole) {
  ...
  const loadAgronomoDashboardData = async () => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (welcomeMsgEl)
          welcomeMsgEl.textContent = `Bem-vindo(a), ${userData.name ? userData.name.split(" ")[0] : "Agrônomo(a)"}!`
  ...
  const renderClientsSummary = (clients) => {
    ...
    clients.slice(0, 3).forEach((client) => {
      // Mostrar os 3 primeiros clientes
      agronomoClientList.innerHTML += `
  ...
  const loadPendingTasksSummary = async (clientIds) => {
    ...
    if (!agronomoPendingTasks) return
    ...
    for (const clientId of clientIds) {
      ...
      tasksSnapshot.forEach((doc) => {
        allPendingTasks.push({
          id: doc.id,
          clientId: clientId,
          clientName: currentClients.find((c) => c.id === clientId)?.name,
          ...doc.data(),
        })
      })
    }
    ...
    allPendingTasks.slice(0, 3).forEach((task) => {
      // Mostrar as 3 primeiras tarefas
  ...
  const loadProfileDetails = async () => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (profileRoleEl) profileRoleEl.textContent = userData.role || "Não informado"
      }
    } catch (error) {
  ...
  const setupEventListeners = () => {
    navButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        if (visitButton) {
          if (view === "visit") {
            const visitDiv = visitButton.querySelector("div")
            if (visitDiv) {
              visitDiv.classList.add("bg-blue-600")
              visitDiv.classList.remove("bg-gray-600")
            }
            ...
            const visitDiv = visitButton.querySelector("div")
            if (visitDiv) {
              visitDiv.classList.remove("bg-blue-600")
              visitDiv.classList.add("bg-gray-600")
            }
          }
        }
    ...
    const navVisitBtn = document.getElementById("navVisitBtnDashboardAgronomo")
    if (navVisitBtn) {
  ...
  const setupDashboardEventListeners = () => {
    ...
    if (viewAllClientsBtn)
      viewAllClientsBtn.addEventListener("click", () => {
        const navClientsBtn = document.getElementById("navClientsBtnDashboardAgronomo")
        if (navClientsBtn) navClientsBtn.click()
      })
    if (viewAllTasksBtn)
  ...
  if (addClientTaskBtn) {
    addClientTaskBtn.addEventListener("click", async () => {
      ...
      const description = modalTaskDescription ? modalTaskDescription.value.trim() : ""
      const propertyId = modalTaskProperty ? modalTaskProperty.value : ""
      const plotId = modalTaskPlot ? modalTaskPlot.value : ""
      ...
      try {
        ...
        if (modalTaskProperty) modalTaskProperty.value = ""
        if (modalTaskPlot) modalTaskPlot.value = ""
        modalTaskDate.value = ""
      ...
      for (const client of currentClients) {
        ...
        propertiesSnapshot.forEach((doc) => {
          ...
          if (modalTaskProperty)
            modalTaskProperty.innerHTML += `<option value="${propData.id}">${propData.name} (${client.name})</option>`
        })
      }
    }
        ...
        if (selectedPropertyId) {
          ...
          if (selectedProperty) {
            const plotsQuery = query(
              collection(db, `clients/${selectedProperty.clientId}/properties/${selectedPropertyId}/plots`),
              orderBy("name"),
            )
  ...
  async function openAddOperatorModal() {
    ...
    if (currentClients.length > 0) {
      currentClients.forEach((client) => {
        if (operatorClientSelect)
          operatorClientSelect.innerHTML += `<option value="${client.id}">${client.name}</option>`
      })
  ...
  async function handleAddOperator(e) {
    e.preventDefault()
    if (!newOperatorNameInput || !newOperatorEmailInput || !newOperatorPasswordInput || !operatorClientSelect) return
