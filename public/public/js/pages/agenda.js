...
function setupAgendaEventListeners() {
  ...
  document.addEventListener("click", (event) => {
    if (
      clientSuggestionsDiv &&
      !clientSuggestionsDiv.contains(event.target) &&
      clientSearchInput &&
...
// Função de debounce para a busca de clientes
function debounceSearchClients() {
  clearTimeout(window.debounceSearchClientsTimeout)
  window.debounceSearchClientsTimeout = setTimeout(async () => {
...
async function fetchTasksForCalendar(start, end) {
  ...
  if (
    activeClientFilter &&
    activeClientFilter.id &&
    (currentUser.userRole === "agronomo" || currentUser.userRole === "admin")
  ) {
      ...
      (snapshot) => {
        const tasks = snapshot.docs.map((doc) => {
          ...
          return {
            ...
            color: data.status === "Concluída" ? "#4CAF50" : data.status === "Pendente" ? "#FFC107" : "#2196F3",
          }
        })
...
async function setupClientSearch() {
  ...
  if (
    !clientSearchInput ||
    !clientSuggestionsDiv ||
    !activeClientFilterDiv ||
    !clientNameDisplay ||
    !clearClientFilterBtn
  ) {
